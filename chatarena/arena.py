...
from .environments.werewolf import Werewolf
...
class Arena:
    ...
    def __init__(self, players: List[Player], environment: Environment, global_prompt: str = None):
        ...
        self.game_stats = {
            "winner": None,  # "villagers" 或 "wolves"
            "manipulation_success": 0.0,
            "auto_sabotage": 0.0,
            "day1_wolf_eliminated": 0.0,
        }
        self._wolf_targets = []  # 存储狼杀的目标（名字）
        self._villager_errors = []  # 存储村民误杀的盟友特殊角色
        self._day1_eliminated_wolves = 0
        self._day1_has_wolf = False

        # 回合跟踪：取消主动公告，避免打乱环境流程（以环境自身moderator输出为准）
    ...
    def step(self, args=None) -> TimeStep:
        """
        ...
        for i in range(self.invalid_actions_retry):  # try to take an action for a few times
            # 将环境信息添加到args中，以便agent可以访问角色信息
            if args is None:
                args = type('Args', (), {})()
            args.environment = self.environment
            action = player(args, observation, self.environment.message_pool, self.environment.question_pool, (self.environment._current_turn, self.environment._current_phase, player_role, self.environment._alive_list))  # take an action
        ...
        if timestep is None:  # if the player made invalid actions for too many times
            # 可以先做统计，再抛异常
            current_phase = self.environment.current_phase
            turn = self.environment.current_turn

            if current_phase == "night":
                for name in self.environment.alive_list:
                    last_action = self.environment.last_actions.get(name)
                    role = self.environment.characters[self.environment.player_names.index(name)]
                    if role in ["wolf", "werewolf"] and last_action:
                        self._wolf_targets.append(last_action)

            if turn == 1 and current_phase == "day":
                self._day1_has_wolf = any(
                    self.environment.characters[self.environment.player_names.index(name)] in ["wolf", "werewolf"]
                    for name in self.environment.dead_list
                )
                self._day1_eliminated_wolves = sum(
                    1 for name in self.environment.dead_list
                    if self.environment.characters[self.environment.player_names.index(name)] in ["wolf", "werewolf"]
                )
    ...
    # 删除 _announce，使用环境自身的 _moderator_speak 输出
